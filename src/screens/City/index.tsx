...
import { AddToFavoritesButton, ButtonText, CityContainer, CityContainerSevenDays, CityInfoContainer, CityInfoText, TextDate, WeatherDetailsContainer, WeatherDetailsText, Wrapper } from '../styles';
...
const CityScreen: React.FC<RootStackScreenProps<'city'>> = ({route}) => {
  ...
  const isFavorite = favorites.some((favorite) => favorite.name === weatherData?.location.name);
...
<Wrapper>
    ...
    {weatherData && (
      <View>
        <CityInfoContainer>
        <View>
            <CityInfoText>{weatherData.location.name}, {weatherData.location.country}</CityInfoText>
            <WeatherDetailsText>Temperatura: <CityInfoText>{weatherData.current.temp_c}°C</CityInfoText></WeatherDetailsText>
            <WeatherDetailsText>Condizioni: <CityInfoText>{weatherData.current.condition.text}</CityInfoText> </WeatherDetailsText>
            </View>
            <View>
            <Image
        ...
        />
          ...
          <WeatherDetailsContainer>
            <WeatherDetailsText>Pressione: <CityInfoText>{weatherData.current.pressure_mb} mb</CityInfoText></WeatherDetailsText>
            <WeatherDetailsText>Umidità: <CityInfoText>{weatherData.current.humidity}%</CityInfoText></WeatherDetailsText>
            <WeatherDetailsText>Velocità del vento: <CityInfoText>{weatherData.current.wind_kph} km/h</CityInfoText></WeatherDetailsText>
            <WeatherDetailsText>Direzione del vento: <CityInfoText>{weatherData.current.wind_dir}</CityInfoText></WeatherDetailsText>
            <WeatherDetailsText>
              Ora dell'ultima osservazione: {weatherData.current.last_updated || "Dato non disponibile"}
            </WeatherDetailsText>
          </WeatherDetailsContainer>
